...
if (isset($_POST["id"]) && !empty($_POST["id"])) {
    ...
    if (empty($name_err) && empty($gender_err) && empty($mobile_err) && empty($age_err) && empty($body_temp_err) && empty($covid_diagnosed_err) && empty($covid_encounter_err) && empty($vaccinated_err) && empty($nationality_err)) {
        ...
        $sql = "UPDATE covid_data SET name=?, gender=?, mobile=?, age=?, body_temp=?, covid_diagnosed=?, covid_encounter=?, vaccinated=? WHERE id=?";

        if ($stmt = mysqli_prepare($link, $sql)) {
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sssssssss", $param_id, $param_name, $param_gender, $param_mobile, $param_age, $param_body_temp, $param_covid_diagnosed, $param_covid_encounter, $param_vaccinated, $param_nationality);
        ...
        if ($stmt = mysqli_prepare($link, $sql)) {
            ...
            if (mysqli_stmt_execute($stmt)) {
                ...
                if (mysqli_num_rows($result) == 1) {
                    ...
                    $name = $row["body_temp"];
                    $address = $row["age"];
                    $salary = $row["covid_diagnosed"];
                    $name = $row["covid_encounter"];
                    $address = $row["vaccinated"];
                    $salary = $row["nationality"];
                } else {
...
<div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <a href="covid19-form.php" class="btn btn-secondary ml-2">Cancel</a>
                    </form>
                </div>
            </div>
        </div>
